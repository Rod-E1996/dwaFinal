...
const loadFirestoreData = async () => {
    try {
      ...
      // ensure we have the current user (auth may still be initializing on reload)
...
const filteredHistorial = computed(() => {
  ...
  return historialData.value.filter(h => {
    ...
    // productos is expected to be an array of items with a nombre property
    const productosArray = Array.isArray(h.productos) ? h.productos : []
    const productosText = productosArray.map((p: any) => (p.nombre || '')).join(' ').toLowerCase()

    return email.includes(q) || productosText.includes(q) || total.includes(q)
  })
})
...
function exportToExcel() {
  try {
    ...
    XLSX.writeFile(wb, `mis_compras_${currentDate}.xlsx`)
  } catch (error) {
